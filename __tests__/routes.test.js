...
describe("POST /auth/login", function () {
  ...
  test("Should not allow bad password to login", async function () {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
...
describe("GET /users", function () {
  ...
  test("should list all users", async function () {
    //changed to .set instead of .send
    const response = await request(app)
      .get("/users")
      .set("_token", `${tokens.u1}`)
    expect(response.statusCode).toBe(200);
...
describe("GET /users/[username]", function () {
  ...
  test("should return 404 for false user", async function () {
    const response = await request(app)
      .get("/users/badUser")
      .set("_token", `${tokens.u1}`);
...
describe("PATCH /users/[username]", function () {
  ...
  test("should deny access if not admin/right user", async function () {
    ...
    const response = await request(app)
      .patch("/users/u1")
      .set("_token", `${tokens.u2}`) // wrong user!
    expect(response.statusCode).toBe(401);
  });
...
describe("DELETE /users/[username]", function () {
  ...
  test("should allow if admin", async function () {
    //change to .set instead of .send
    const response = await request(app)
      .delete("/users/u1")
      .set("_token", `${tokens.u3}`) // u3 is admin
    expect(response.statusCode).toBe(200);
